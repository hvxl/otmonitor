...
#-----------------------------------------------------------------------------
# asnBigInteger : Encode a long integer value
#-----------------------------------------------------------------------------

proc ::asn::asnBigInteger {bignum} {
    set len [expr {[string length [format %+llx $bignum]] / 2}]
    if {$bignum < 0} {
        # generate two's complement form
	set bignum [expr {$bignum + (1 << ($len * 8))}]
	set hex [format %0*llx [expr {$len * 2}] $bignum]
        if {([scan [string index $hex 0] %x] & 8) == 0} {
    ...
    return [binary format H2a*H* 02 [asnLength $len] $hex]
}
...
proc ::asn::asnGetBigInteger {data_var bignum_var} {
	...
	binary scan [string index $integerBytes 0] cu head
