...
proc ::pki::rsa::serialize_key {keylist} {
	...
	set ret [::asn::asnSequence \
			[::asn::asnBigInteger 0] \
			[::asn::asnBigInteger $key(n)] \
			[::asn::asnBigInteger $key(e)] \
			[::asn::asnBigInteger $key(d)] \
			[::asn::asnBigInteger $key(p)] \
			[::asn::asnBigInteger $key(q)] \
			[::asn::asnBigInteger $e1] \
			[::asn::asnBigInteger $e2] \
			[::asn::asnBigInteger $c] \
	]
...
proc ::pki::x509::create_cert {signreqlist cakeylist serial_number notBefore notAfter isCA extensions {encodePem 0} {algo "sha1"}} {
	...
	lappend certlist [::asn::asnBigInteger $serial_number]
	...
	switch -- $type {
		"rsa" {
			set pubkey [::asn::asnSequence \
				[::asn::asnBigInteger $signreq(n)] \
				[::asn::asnBigInteger $signreq(e)] \
			]
